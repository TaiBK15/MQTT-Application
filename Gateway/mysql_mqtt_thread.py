...
def listen_and_publish(mess):
	...
	while True:
	    ...
	    threadLock.acquire()
	    saveToDatabase("device_1", "30", "60", "50")
	    saveToDatabase("device_2", "40", "50", "70")
	    json_obj = conv_to_json("serv/device_1", "device_1", 0x30, 0x25, 0x15)
	    mqttclient.publish("gw/device_1/data", str(json_obj))
	    threadLock.release()
...
def saveToDatabase(device_ID, temp, bright, humidity):
	"""
	...
	mysqlQuery.insertTable(device_ID, temp, bright, humidity)	
